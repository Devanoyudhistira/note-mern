...
export default function Editfooter({editmode,editmodeactive,updatenote}){
    ...
    <div className="justify-self-end self-end absolute bottom-0 px-2 py-1 mt-3 h-max w-screen border-t-2 border-black bg-red-500 flex justify-end " >
        ...
        {!editmode && <motion.button onClick={editmodeactive} className="text-center rounded-full -mt-6 p-3 w-max h-max bg-blue-400 border-2 border-black" >
          <PencilSquare className="text-4xl " />
        </motion.button>}
        {editmode && <motion.button onClick={updatenote} className="text-center rounded-full -mt-6 p-3 w-max h-max bg-blue-400 border-2 border-black" >
